...
date_default_timezone_set("America/Matamoros");
  ...
  if($_POST){
    ...
    $unidad = "pieza";
            ...
            //Ganancia sucursal Pedro

            $traer_id = $con->prepare("SELECT id FROM `ventas` WHERE WEEK(Fecha) = ? AND YEAR(Fecha) = ? AND tipo = ? AND estatus <> ? AND WEEKDAY(Fecha) =? AND id_Sucursal =?");
            $traer_id->bind_param('ssssss', $semana, $año, $tipo, $estatus, $hoy, $sucursalS);
            $traer_id->execute();
            $resultado = $traer_id->get_result();
             $costo_acumuladoS = 0;
             if($resultado->num_rows < 1){
                    ...
                    // echo "sin valores"; 
                     ...
                     if($array_costos->num_rows < 1){
                         ...
                         while($fila3 = $array_costos->fetch_assoc()){
                             $costo_unitario = $fila3["precio_Inicial"];
                             $costo_total = $costo_unitario * $cantidad;
                             $costo_acumuladoS = $costo_acumuladoS + $costo_total;
            ...
            $traer_id->close();
               ...
               $ganancia_domingo_sql = $con->prepare("SELECT SUM(Total) FROM `ventas` WHERE WEEK(Fecha) = ? AND YEAR(Fecha) = ? AND tipo = ? AND estatus <> ? AND WEEKDAY(Fecha) =? AND id_Sucursal =?");
               $ganancia_domingo_sql->bind_param('ssssss', $semana, $año, $tipo, $estatus, $hoy, $sucursalP);
               ...
               $ganancia_domingo_sql = $con->prepare("SELECT SUM(Total) FROM `ventas` WHERE WEEK(Fecha) = ? AND YEAR(Fecha) = ? AND tipo = ? AND estatus <> ? AND WEEKDAY(Fecha) =? AND id_Sucursal =?");
               $ganancia_domingo_sql->bind_param('ssssss', $semana, $año, $tipo, $estatus, $hoy, $sucursalS);
               $ganancia_domingo_sql->execute();
               ...
               if($ganancia_sendero == null){
                ...
                $ganancia_sendero = $ganancia_sendero - $costo_acumuladoS;
            }
