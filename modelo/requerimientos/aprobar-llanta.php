...
//Comprobar que no exista un movimiento ya hecho para este requerimiento
    $traercantidad = "SELECT dr.cantidad, dr.id_requerimiento, r.id_usuario, r.id_sucursal, r.id_movimiento FROM detalle_requerimientos dr INNER JOIN requerimientos r ON dr.id_requerimiento = r.id WHERE dr.id = ?";
    $result = $con->prepare($traercantidad);
    $result->bind_param('s', $id_detalle);
    $result->execute();
    $result->bind_result($total_llantas, $id_requerimiento, $id_usuario, $sucursal_id, $id_movimiento);
    $result->fetch();
    $result->close();
    ...
    $hay_movimiento_vacio = empty($id_movimiento);

    if($hay_movimiento_vacio){
    ...
    $respp->execute();
    $respp->close();
    $update = "UPDATE detalle_requerimientos SET estatus = 3 WHERE id = ?";
    $respp = $con->prepare($update);
