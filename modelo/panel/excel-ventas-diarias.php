...
//cabezera
        ...
        $estatus = "Pagado";
        $tipo = "Normal";
        $consultaVentas = "SELECT COUNT(*) FROM ventas WHERE id_sucursal=? AND fecha = ? AND estatus =? AND tipo =?";
        $resp = $con->prepare($consultaVentas);
        $resp->bind_param('ssss', $id_sucursal, $fecha, $estatus, $tipo);
        $resp->execute();
        ...
        $resp->close();
                ...
                if($numero_ventas > 0){
                    $traer_venta = "SELECT * FROM ventas WHERE id_sucursal = '$id_sucursal' AND fecha = '$fecha' AND estatus ='$estatus' AND tipo ='$tipo'";
