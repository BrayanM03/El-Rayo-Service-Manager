...
if (!isset($_SESSION['id_usuario'])) {
    ...
    responder(false, 'El tiempo de la sesión expiró, recarga la pagina porfavor', [], true);
}else{
    ...
    if (isset($_POST)) {
        ...
        if ($total_productos <= 0) {
            responder(false, 'No hay productos en preventa, revisa el carrito o recarga la pagina para ver si estan los productos', 'warning', [], true);
        }
        ...
        if ($stock_ok) {
                ...
                try {
                    ...
                    if($comprobacion_venta['estatus'] == false ){
                        $comprobacion_estatus = $clase_venta->establecerEstatus($id_Venta, 'Cancelada', 5);
