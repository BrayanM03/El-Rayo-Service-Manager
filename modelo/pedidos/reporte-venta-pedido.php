...
$ID = $con->prepare("SELECT a.id, a.id_sucursal, a.id_usuario, c.Nombre_Cliente, c.Telefono, a.abonado, a.restante, a.total,  a.tipo, a.estatus, a.hora_inicio, a.comentario, a.fecha_inicio, a.fecha_final FROM pedidos a INNER JOIN clientes c ON a.id_cliente = c.id WHERE a.id_venta = ?");
$ID->bind_param('i', $idVenta);
$ID->execute();
$ID->bind_result($id_apartados, $sucursal, $vendedor_id, $cliente, $telefono_cliente, $primer_abono, $restante, $total, $tipo, $estatus, $hora, $comentario, $fecha_inicio, $fecha_final);
$ID->fetch();
$ID->close();

$ID = $con->prepare("SELECT pago_efectivo, pago_tarjeta, pago_transferencia, pago_cheque, pago_sin_definir FROM abonos_pedidos WHERE id_pedido = ? AND estado = 0"); //El 0 indica el estado de apono de liquidacion
...
$select = "SELECT SUM(abono) FROM abonos_pedidos WHERE id_pedido = ?";
...
$select = "SELECT MAX(id) FROM abonos_pedidos WHERE id_pedido = ?";
$re = $con->prepare($select);
$re->bind_param('i', $id_apartados);
$re->execute();
...
//Obtener el ultimo abono del apartadi
$select = "SELECT abono FROM abonos_pedidos WHERE id = ?";
$re = $con->prepare($select);
...
function cuerpoTabla(){
    ...
    if($total == 0){
        ...
        $detalle = $conexion->prepare("SELECT llantas.Modelo, da.Cantidad,llantas.Descripcion, llantas.Marca, da.precio_Unitario, da.Importe FROM detalle_pedido da INNER JOIN llantas ON da.id_Llanta = llantas.id WHERE da.id_pedido = ?");
       ...
       // $pdf->SetAlpha(0.2);
        ...
        $detalle = $conexion->prepare("SELECT llantas.modelo, da.cantidad,llantas.Descripcion, llantas.Marca, da.precio_unitario, da.importe FROM detalle_pedido da INNER JOIN llantas ON da.id_llanta = llantas.id WHERE da.id_pedido = ?  AND da.modelo != 'no aplica'");
        $detalle->bind_param('i', $id_apartados);
        $detalle->execute();
        $resultado = $detalle->get_result(); 
        $detalle->close(); 
...
/*     $pdf->Ln(10);
   ...
   /*  $pdf->Image('../../../vistas/dist/img/QR_code.png',20,238,35); */
    ...
    $pdf->Output("Venta de Pedido RAY" . $_GET["id"] .".pdf", "I");
}
