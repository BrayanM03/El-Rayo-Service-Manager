...
$ID = $con->prepare("SELECT  a.fecha, a.hora, a.pago_efectivo, a.pago_tarjeta, a.pago_transferencia, a.pago_cheque, a.pago_sin_definir, a.estado FROM abonos_pedidos a WHERE a.id = ?");
...
function cuerpoTabla(){
    ...
    $total = 0;
        ...
        $detalle = $conexion->prepare("SELECT COUNT(*) FROM detalle_pedido dp INNER JOIN llantas ON dp.id_llanta = llantas.id WHERE dp.id_pedido = ?");
        $detalle->bind_param('i', $id_venta);
        $detalle->execute();
        $detalle->bind_result($total); 
        $detalle->close();  
