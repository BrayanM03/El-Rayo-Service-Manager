...
//Creditos
function procesarFiltrosCreditos(){
  var tabla_ventas = $('#creditos');
  ...
  let filtro_estatus = $("#filtro-estatus").val() //Multiple values
  let filtro_asesor = $("#buscador-asesor").val() //Multiple values
  let plazo = $("#filtro-plazo").val()
  let estatus = $("#filtro-estatus").val()



  table.destroy()
  if ($.fn.DataTable.isDataTable( '#creditos' ) ) {
      tabla_ventas.destroy();
    }
  tabla_ventas.empty();
  tabla_ventas.append(`
  ...
  $.ajax({
      ...
      data: {folio, fecha_final, fecha_inicial, fecha_vencimiento_inicial, fecha_vencimiento_final, sucursal, vendedor, cliente, marca_llanta, ancho_llanta, alto_llanta, rin_llanta, filtro_estatus, filtro_tipo, filtro_asesor, plazo, estatus},
      dataType: "JSON",
      success: function (response) {
