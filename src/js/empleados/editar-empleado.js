toastr.options = {
  ...
  let eliminar_foto = false;
  let cargar_foto =false;
  function guardarDatosGenerales(id_empleado){
    ...
    if(validacionFormularioEmpleado()){
      ...
      //Cargando foto empleado
      if(cargarFoto){
        let fileInput = document.getElementById('foto-perfil');
        if (fileInput.files.length > 0) {
            formData.append('foto_perfil', fileInput.files[0]);
        }
      }

      formData.append('cargar_foto', cargar_foto)
      formData.append('eliminar_foto', eliminar_foto)
      ...
      $.ajax({
        ...
        contentType: false,
        processData: false,
        url: "./modelo/empleados/actualizar-empleado.php",
        data: formData,
        dataType: "json",
  ...
  function cargarHorario(tipo, avanzar_){
    /* if(avanzar_==3){
      ...
      <div style="height: ${card_height}px">
          <div class="row mt-3 justify-content-center">
                ...
                <div class="col-4">
                    ...
                    <div class="btn btn-info" onclick="siguienteProceso(3, ${contador_paso_3})">Guardar horario</div>
                </div>
            </div>
  ...
  function mostrarDocumentosEmpleado() {
  ...
  <div class="row text-center mt-4 justify-content-center">
                  <div class="col-4">
                      ...
                      <div class="btn btn-success" onclick="guardarDocumentosAdjuntos(1)">Guardar documentos adjuntos</div>
                  </div>
              </div>
  `);
  ...
  let documentos_ = ["cv", "curp", "domicilio", "nss", "id", "contrato", "bancarios"];
    ...
    function cargarDatosGenerales(tipo, avanzar_otra_vez){
      let breadcrumbs = document.querySelectorAll('.breadcrumbs__item')
      breadcrumbs.forEach(element => {
        element.classList.remove('is-active')
      });
